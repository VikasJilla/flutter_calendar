...
class _CustomCalendarState extends State<CustomCalendar> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        color: Colors.white,
        child: buildCalendarWithEvents(context)
      );
  }
          ...
          for(String day in getWeekDays())
            SizedBox(
              ...
              child:Container(
                ...
                child: Text(day,style: Theme.of(context).textTheme.display1.copyWith(color:Colors.black),overflow: TextOverflow.ellipsis,maxLines: 1,)
              )
            )
        ],
      ),
...
/////////////////////////////////////////
  ...
  void initDimensions(){
    if(widget.calendarSize == null){
      ...
      itemHeight = (size.height - kBottomNavigationBarHeight - kToolbarHeight - 80) / getNumberOfWeeksInMonth(_currentDate);
